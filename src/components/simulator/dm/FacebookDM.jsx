      ...
      <div className="flex items-center gap-1.5">
        <div className="flex items-end gap-0.5 h-3">
          {[1, 2, 3, 4].map((bar) => (
            <div
              key={bar}
              className={`w-0.5 rounded-full ${bar <= bars ? 'bg-black' : 'bg-black/30'}`}
              style={{ height: `${bar * 25}%` }}
            />
          ))}
        </div>
        ...
        <div className="w-6 h-3 border border-black rounded-sm relative">
          <div className="absolute inset-0.5 bg-black rounded-sm" style={{ width: `${battery}%` }} />
          <div className="absolute -right-0.5 top-1/2 -translate-y-1/2 w-0.5 h-1.5 bg-black rounded-r-sm" />
        </div>
      </div>
    </div>
  );
...
export default function FacebookDM({ account1, account2, visibleMessages, isPlaying, statusBar = { time: '9:41', signal: 4, battery: 80 } }) {
